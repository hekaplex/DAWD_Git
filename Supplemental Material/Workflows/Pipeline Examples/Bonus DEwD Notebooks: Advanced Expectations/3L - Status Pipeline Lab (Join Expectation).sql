...
-- MAGIC %md
-- MAGIC ## BONUS: Defining Tables for Advanced Validation
-- MAGIC
-- MAGIC Complex data quality checks examples
-- MAGIC * A derived table contains all records from the source table
-- MAGIC * Guaranteeing the equality of a numeric column across tables
-- MAGIC **Solution:**
-- MAGIC Define DLT using aggregate and join queries and use the results of those queries as part of your expectation checking.
-- MAGIC

-- COMMAND ----------

CREATE OR REFRESH LIVE TABLE comparison_email_updates
(
  CONSTRAINT no_missing_records
  EXPECT (silver_key IS NOT NULL AND order_key IS NOT NULL)  
)
AS
SELECT a.order_id as silver_key, b.order_id as order_key
FROM LIVE.status_silver a
FULL OUTER JOIN LIVE.subscribed_order_emails_v b
ON a.order_id = b.order_id;
